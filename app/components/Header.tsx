...
export default function Header({ 
  ...
  onSearchClick,
  onRefreshCalendar,
  calendarName: initialCalendarName,
}: HeaderProps) {
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [calendarName, setCalendarName] = useState(initialCalendarName);
  const location = useLocation();
  const calendarId = new URLSearchParams(location.search).get('calendarId');
  
  // Update calendar name when prop changes
  useEffect(() => {
    setCalendarName(initialCalendarName);
  }, [initialCalendarName]);
  ...
  useEffect(() => {
    const handleFavoriteChange = (event: Event) => {
      ...
      if (detail.type === 'update' && detail.calendarId === calendarId) {
        setCalendarName(detail.updatedCalendar.name);
      }
    };
